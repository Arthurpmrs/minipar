...
for epoch in range(20000):
    for i in range(len(inputs)):
        # Fase de feedforward
        hidden_layer_input = [
            inputs[i][j] * weights_input_hidden[j][k]
            for k in range(3)
            for j in range(2)
        ]
        hidden_layer_output = [
            sigmoid(
                sum(hidden_layer_input[i * 2 : (i + 1) * 2]) + bias_hidden[i]
            )
            for i in range(3)
        ]
        output_layer_input = (
            sum([
                hidden_layer_output[j] * weights_hidden_output[j]
                for j in range(3)
            ])
            + bias_output
        )
        ...
        weights_hidden_output = [
            weights_hidden_output[j]
            + hidden_layer_output[j] * d_predicted_output * learning_rate
            for j in range(3)
        ]
...
for i in range(len(inputs)):
    ...
    output_layer_input = (
        sum([
            hidden_layer_output[j] * weights_hidden_output[j] for j in range(3)
        ])
        + bias_output
    )
    predicted_output = sigmoid(output_layer_input)
