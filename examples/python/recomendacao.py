...
def calculate_similarity(user1_ratings, user2_ratings):
    ...
    product_sum = sum(user1_ratings[product] * user2_ratings[product] for product in common_products)
...
def get_recommendations(user_ratings, target_user):
    ...
    for other_user in user_ratings:
        if other_user == target_user:
            continue
        similarity = calculate_similarity(user_ratings[target_user], user_ratings[other_user])
        if similarity <= 0:
            continue
    ...
    rankings = [(total / sim_sums[product], product) for product, total in totals.items() if sim_sums[product] != 0]
...
def main():
    ...
    while True:
        ...
        target_user = input("Digite o usuário para o qual deseja recomendações (ou 'sair' para finalizar): ").strip()
